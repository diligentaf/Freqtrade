...
class BBRSIHyperopt(IHyperOpt):
    ...
    """
    # MOOG
    # Testing various strategies to find the optimal sharp ratio
    @staticmethod
    ...
    def indicator_space() -> List[Dimension]:
        ...
        """
        # MOOG
        # These are the ranges of parameters to test
        # Testing out rsi between 5 to 50 && testing the combinations of the other parameters to find the optimal rsi
        return [
            Integer(5, 50 , name='rsi-value'),
            # MOOG
            # Try using rsi as a signal and without it
            # Do I need this indicator or not?
            Categorical([True, False], name='rsi-enabled'),
            # MOOG
            # Which bb should we use? The first? second? or third or fourth?
    ...
    def buy_strategy_generator(params: Dict[str, Any]) -> Callable:
        ...
        def populate_buy_trend(dataframe: DataFrame, metadata: dict) -> DataFrame:
            ...
            # GUARDS AND TRENDS
            # MOOG
            # if rsi-enabled is True, run this
            if params.get('rsi-enabled'):
                # MOOG
                # When the buy is greater than the rsi-value, buy
                conditions.append(dataframe['rsi-buy'] > params['rsi-value'])

            # TRIGGERS
            # MOOG
            # This is another condition where the program witll test out different strategies
